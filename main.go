...
var gravityProgram, axisProgram, sphereProgram, axisVertexArray, sphereVertexArray, sphereInstanceColorBuffer, sphereInstanceModelBuffer, gravityOrbBuffer0, gravityOrbBuffer1, query uint32
...
//	)
	...
	for localWorkGroupSize = 32; localWorkGroupSize <= 2048; localWorkGroupSize *= 2 {
		...
		for numSpheres := 2; numSpheres <= 262144; numSpheres *= 2 {
			...
			fmt.Printf("starting profiling run with local workgroup size of %v and %v spheres\n", localWorkGroupSize, numSpheres)
			...
			var orbs []Orb = make([]Orb, numSpheres)
			var orbMassLocations []mgl.Vec3 = make([]mgl.Vec3, numSpheres)
			var sumOrbMass float32
			var sumOrbMassLocations mgl.Vec3
			orbs[0].location = mgl.Vec3{0, 0, 0}
			orbs[0].mass = 1e11
			orbMassLocations[0] = orbs[0].location.Mul(orbs[0].mass)
			sumOrbMass = orbs[0].mass
			sumOrbMassLocations = orbMassLocations[0]
			for i := 1; i < numSpheres; i++ {
				orbs[i].location = mgl.Vec3{
					rand.Float32() - 0.5,
					(rand.Float32() - 0.5) * 0.05,
					rand.Float32() - 0.5,
				}.Normalize().Mul(1000.0 + rand.Float32() * 21000.0)

				orbs[i].mass = float32(math.Pow10(rand.Intn(3))) * rand.Float32()

				orbMassLocations[i] = orbs[i].location.Mul(orbs[i].mass)

				sumOrbMass += orbs[i].mass

				sumOrbMassLocations = sumOrbMassLocations.Add(orbMassLocations[i])
			}

			orbs[0].velocity = mgl.Vec3{0, 0, 0}
			for i := 1; i < numSpheres; i++ {
				// displacement vector from barycenter (without current orb) to current orb
				dv := orbs[i].location.Sub(sumOrbMassLocations.Sub(orbMassLocations[i]).Mul(1 / (sumOrbMass - orbs[i].mass)))

				// velocity magnitude
				mag := ((sumOrbMass - orbs[i].mass) / sumOrbMass) * float32(math.Sqrt(float64((G * sumOrbMass) / dv.Len())))

				// velocity direction
				dir := dv.Cross(mgl.Vec3{0, 1, 0}).Normalize()

				// initial velocity
				orbs[i].velocity = dir.Mul(mag)
			}


			// copy orb positions, masses and velocities into shader storage buffers for use by the shaders
			gl.DeleteBuffers(1, &gravityOrbBuffer0)
			gl.CreateBuffers(1, &gravityOrbBuffer0)
			{
				// populate new buffer with data from data pointer of slice variable 'orbs'
				shdr := (*reflect.SliceHeader)(unsafe.Pointer(&orbs))
				gl.NamedBufferStorage(gravityOrbBuffer0, numSpheres * 2 * 4 * 4, unsafe.Pointer(shdr.Data), 0)
			}
			gl.BindBufferBase(gl.SHADER_STORAGE_BUFFER, 0, gravityOrbBuffer0)

			// theoretically there's no need to populate this buffer since the newly calculated positions are going to be copied here with the first compute dispatch
			gl.DeleteBuffers(1, &gravityOrbBuffer1)
			gl.CreateBuffers(1, &gravityOrbBuffer1)
			gl.NamedBufferStorage(gravityOrbBuffer1, numSpheres * 2 * 4 * 4, nil, 0)
			gl.BindBufferBase(gl.SHADER_STORAGE_BUFFER, 1, gravityOrbBuffer1)
			...
			for i := 0; i < profilingLogLength; i++ {
				...
				if orbBuffer1Active {
					...
					gl.BindBufferBase(gl.SHADER_STORAGE_BUFFER, 0, gravityOrbBuffer1)
					gl.BindBufferBase(gl.SHADER_STORAGE_BUFFER, 1, gravityOrbBuffer0)
				}
				orbBuffer1Active = !orbBuffer1Active
			...
			fileName := fmt.Sprintf("b%v_s%v.csv", localWorkGroupSize, numSpheres)
